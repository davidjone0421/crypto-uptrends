...
select v.slug, v.title, v.excerpt, v.content, v.cover_image, c.id, v.author, v.read_minutes, v.is_featured
from (values
  ('bitcoin-reclaims-key-resistance-as-etf-inflows-surge',
   ...
   'bitcoin-news', 'Marcus Chen', 5, true),
  ('ethereum-pectra-upgrade-what-changes-for-stakers',
   ...
   'altcoin-updates', 'Priya Nair', 6, true),
  ...
  ('btc-price-prediction-q1-2026-analysts-divided',
   ...
   E'Bitcoin price forecasts for the first quarter of 2026 are diverging more than at any point in the last cycle, with reputable analysts publishing targets that span nearly 100% in either direction.\n\nThe bull case rests on three pillars: continued ETF accumulation, a weakening US dollar, and the ongoing supply shock from the April halving. The bear case emphasizes overheated derivatives markets, concentration risk in a handful of large holders, and the possibility that macro conditions sour faster than expected.',
   'https://images.unsplash.com/photo-1605792657660-596af9009e82?w=1200&q=80',
   'price-predictions', 'Jordan Hale', 8, false),
  ('stablecoins-cross-200b-supply-milestone',
   ...
   E'The total supply of stablecoins has surpassed $200 billion for the first time, marking a milestone for what has become one of crypto''s most important product categories.\n\nUSDT and USDC continue to dominate, but the fastest growth has come from yield-bearing alternatives backed by short-term US Treasuries. Emerging markets remain a key driver. In Argentina, Turkey, and Nigeria, stablecoins are increasingly used as a savings instrument and a medium of exchange.',
   'https://images.unsplash.com/photo-1621761191319-c6fb62004040?w=1200&q=80',
   'bitcoin-news', 'Sofia Reyes', 5, false),
  ('layer-2-wars-base-vs-arbitrum-vs-optimism',
   ...
   E'The competitive dynamics among Ethereum Layer 2 networks have shifted dramatically over the past six months, with Coinbase''s Base now leading on several key metrics. Base has captured the lion''s share of consumer onchain activity, particularly in social and creator economy applications. Arbitrum continues to dominate in DeFi TVL.',
   'https://images.unsplash.com/photo-1639322537228-f710d846310a?w=1200&q=80',
   'altcoin-updates', 'Marcus Chen', 6, false),
  ('ai-tokens-rally-as-compute-demand-explodes',
   ...
   E'AI-focused crypto tokens have led the market higher in recent weeks, as decentralized compute networks report exploding demand from both Web3 and traditional AI customers. Protocols offering distributed GPU infrastructure have seen utilization rates above 90%, prompting fresh capacity expansion and waitlists for premium hardware.',
   'https://images.unsplash.com/photo-1677442136019-21780ecad995?w=1200&q=80',
   'ai-web3', 'Damon Wright', 5, false),
  ('ethereum-price-prediction-can-eth-reclaim-5k',
   ...
   'price-predictions', 'Priya Nair', 7, false)
) as v(slug, title, excerpt, content, cover_image, cat_slug, author, read_minutes, is_featured)
